 ...
 * Le pseudo peut être remplacé par celui que porte l'adresse aujourd'hui
 * (voir currentPseudos dans lib/identity.js) : c'est toujours le dernier
 * pseudo confirmé qui s'affiche, y compris sur les anciens messages.
 */
export function publicComment(c, pseudo = null) {
  return {
    id: c.id,
    ride: c.ride,
    stage: c.stage || null,
    parentId: c.parentId || null,
    pseudo: pseudo || c.pseudo,
    body: c.body,
...
export async function deleteSubmission(submissionId) {
  ...
  // Une modification en attente sur une sortie supprimée n'a plus d'objet :
  // sans ce nettoyage elle resterait affichée dans /admin sans sortie à mettre
  // à jour.
  await storage.del(`revisions/${submissionId}`);
}

// --- Modifications proposées par l'auteur -----------------------------------
// Clé : revisions/<submissionId>
//
// Une sortie déjà publiée n'est jamais réécrite directement par son auteur : sa
// version retouchée attend ici la relecture. La page en ligne ne bouge pas tant
// que la modification n'a pas été acceptée depuis /admin.
//
// Ces enregistrements vivent sous leur propre préfixe : le script de build ne
// lit que submissions/, il ne voit donc jamais une version en attente.

export async function saveRevision(revision) {
  await storage.put(`revisions/${revision.submissionId}`, revision);
  return revision;
}

export async function getRevision(submissionId) {
  return storage.get(`revisions/${submissionId}`);
}

export async function listRevisions() {
  const all = await storage.list("revisions");
  return all.sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""));
}

export async function deleteRevision(submissionId) {
  await storage.del(`revisions/${submissionId}`);
}
