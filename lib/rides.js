...
import { stageColor } from "./activities";
...
/** Accepte ["Sacoches"] ou [{ emoji: "🚲", label: "Vélos" }]. */
function normalizeGear(raw) {
  if (!Array.isArray(raw) || raw.length === 0) return null;
  const items = raw
    .map((item) => {
      if (typeof item === "string") {
        return item.trim() ? { emoji: null, label: item.trim() } : null;
      }
      if (item && item.label) {
        return { emoji: item.emoji || null, label: item.label };
      }
      return null;
    })
    .filter(Boolean);
...
export function getRideSummaries() {
  return getRideSlugs()
    .map((slug) => {
      ...
      return { slug, ...rest, ...totals(stages) };
    })
...
export function getRideDetail(slug) {
  ...
  const stages = loadStages(slug, info);
  const { declaredStages, ...rest } = info;
  return { slug, ...rest, stages, ...totals(stages) };
}
