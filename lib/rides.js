...
import { stageColor, truncate } from "./activities.js";
import { normalizeGearItems } from "./gear.js";
...
function loadInfo(slug) {
  ...
  // Un info.json retouché à la main peut être mal formé. Dans ce cas la sortie
  // repart sur ses valeurs par défaut, avec un avertissement : le build ne doit
  // pas s'arrêter sur une virgule en trop dans un seul fichier.
  if (fs.existsSync(infoPath)) {
    try {
      raw = JSON.parse(fs.readFileSync(infoPath, "utf-8"));
    } catch (err) {
      console.warn(`info.json illisible, valeurs par défaut : ${slug} (${err.message})`);
      raw = {};
    }
  }
...
/**
 * Établit la liste ordonnée des fichiers GPX d'une sortie.
 * Un bloc "stages" dans info.json fait foi. Sinon tous les .gpx du dossier sont
 * repris dans l'ordre naturel des noms, si bien que etape-1.gpx puis
 * etape-2.gpx fonctionne sans rien déclarer.
 */
function resolveStageFiles(slug, info) {
  if (info.declaredStages && info.declaredStages.length > 0) {
    return info.declaredStages.map((stage, i) => ({
      file: stage.file,
      title: stage.title || `Étape ${i + 1}`,
  ...
  return files.map((file, i) => ({
    file,
    title: files.length > 1 ? `Étape ${i + 1}` : "",
    description: "",
    lodging: null,
  }));
}
...
// Résumé allégé pour la grille d'accueil : les listes de points sont écartées
...
// Détail complet d'une sortie, points de trace de chaque étape compris
...
export async function getRideAuthor(slug) {
  ...
  if (info.submissionId) {
    try {
      const { getSubmission } = await import("./store.js");
