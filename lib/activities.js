// ---------------------------------------------------------------------------
// Types d'activité (champ "activity")
// ---------------------------------------------------------------------------
export const ACTIVITIES = {
  velo: { emoji: "🚲", label: "Vélo" },
  vtt: { emoji: "🚵", label: "VTT" },
  rando: { emoji: "🥾", label: "Randonnée" },
  marche: { emoji: "🚶", label: "Marche" },
  gravel: { emoji: "🚴", label: "Gravel" },
  kayak: { emoji: "🛶", label: "Kayak" },
  ski: { emoji: "⛷️", label: "Ski" },
  raquettes: { emoji: "🌨️", label: "Raquettes" },
  cheval: { emoji: "🐴", label: "Cheval" },
  autre: { emoji: "🧭", label: "Autre" },
};

const ACTIVITY_FALLBACK = { emoji: "🧭", label: "Sortie" };

export function getActivity(key) {
  if (!key) return ACTIVITIES.velo;
  return ACTIVITIES[String(key).toLowerCase()] || ACTIVITY_FALLBACK;
}

// ---------------------------------------------------------------------------
// Fond de carte proposé d'office
//
// Le visiteur garde les deux boutons Relief et Plan : on choisit seulement
// celui qui s'affiche en arrivant sur la fiche, selon l'activité. Le relief va
// aux activités à pied, où les courbes de niveau disent quelque chose ; tout le
// reste prend le plan, le fond le moins encombré.
// Les clés doivent rester celles de LAYERS dans components/RouteMap.jsx.
// ---------------------------------------------------------------------------
export const ACTIVITY_MAP_LAYER = {
  velo: "plan",
  vtt: "plan",
  gravel: "plan",
  rando: "topo",
  marche: "topo",
  raquettes: "topo",
  ski: "topo",
  kayak: "plan",
  cheval: "plan",
  autre: "plan",
};

/** Fond retenu quand l'activité est inconnue : le plan, neutre et toujours lisible. */
export const MAP_LAYER_FALLBACK = "plan";

/** Fond de carte à afficher d'office pour une activité donnée. */
export function mapLayerFor(key) {
  if (!key) return MAP_LAYER_FALLBACK;
  return ACTIVITY_MAP_LAYER[String(key).toLowerCase()] || MAP_LAYER_FALLBACK;
}

// ---------------------------------------------------------------------------
// Niveaux de difficulté
// ---------------------------------------------------------------------------
export const DIFFICULTIES = [
  "Très facile",
  "Facile",
  "Modérée",
  "Sportive",
  "Difficile",
];

/**
 * Rang d'une difficulté, du plus simple au plus engagé.
 *
 * Les sorties les plus anciennes ont pu être saisies avec un intitulé qui n'est
 * plus proposé dans le formulaire. Une valeur inconnue n'est donc pas écartée :
 * elle est simplement rejetée en fin de liste, où elle reste sélectionnable.
 */
export function difficultyRank(value) {
  const index = DIFFICULTIES.indexOf(value);
  return index === -1 ? DIFFICULTIES.length : index;
}

/** Trie des difficultés dans l'ordre du formulaire, l'inconnu à la fin par ordre alphabétique. */
export function sortDifficulties(list) {
  return [...list].sort(
    (a, b) => difficultyRank(a) - difficultyRank(b) || a.localeCompare(b, "fr")
  );
}

// ---------------------------------------------------------------------------
// Types de logement
// ---------------------------------------------------------------------------
export const LODGINGS = {
  bivouac: { emoji: "🏕️", label: "Bivouac" },
  camping: { emoji: "⛺", label: "Camping" },
  wtmg: { emoji: "🌻", label: "Welcome To My Garden" },
  warmshowers: { emoji: "🚿", label: "Warmshowers" },
  hotel: { emoji: "🏨", label: "Hôtel" },
  gite: { emoji: "🏡", label: "Gîte" },
  refuge: { emoji: "🛖", label: "Refuge" },
  auberge: { emoji: "🛏️", label: "Auberge de jeunesse" },
  amis: { emoji: "🏠", label: "Chez des amis" },
  van: { emoji: "🚐", label: "Van ou camping-car" },
  train: { emoji: "🚆", label: "Train de nuit" },
  maison: { emoji: "🏘️", label: "Retour à la maison" },
};

const LODGING_FALLBACK = { emoji: "🛌", label: "Logement" };

export function getLodging(key) {
  if (!key) return LODGING_FALLBACK;
  return LODGINGS[String(key).toLowerCase()] || LODGING_FALLBACK;
}

// ---------------------------------------------------------------------------
// Participants
//
// Le pictogramme masculin ou féminin est tiré au hasard, à partir d'une
// empreinte du contexte (le plus souvent le slug de la sortie). Le tirage est
// donc varié d'une sortie à l'autre mais stable d'un rechargement à l'autre :
// le serveur et le navigateur affichent la même chose, sans quoi React
// signalerait une différence d'hydratation et l'emoji clignoterait.
//
// Ce choix est expliqué dans les mentions légales.
// ---------------------------------------------------------------------------
export const PARTICIPANT_EMOJI = {
  adults: ["👩", "👨"],
  children: ["👧", "👦"],
};

/** Empreinte entière positive et stable d'une chaîne (variante de FNV-1a). */
function fingerprint(text) {
  let hash = 2166136261;
  const value = String(text || "");
  for (let i = 0; i < value.length; i += 1) {
    hash ^= value.charCodeAt(i);
    hash = Math.imul(hash, 16777619);
  }
  return Math.abs(hash);
}

/** Tire le pictogramme d'un groupe, au hasard mais toujours le même pour un seed donné. */
export function participantEmoji(group, seed = "") {
  const list = PARTICIPANT_EMOJI[group] || PARTICIPANT_EMOJI.adults;
  return list[fingerprint(`${seed}|${group}`) % list.length];
}

/** "2 adultes", "1 enfant" : le pluriel suit le nombre. */
export function participantLabel(group, count) {
  const singular = group === "children" ? "enfant" : "adulte";
  return `${count} ${singular}${count > 1 ? "s" : ""}`;
}

/**
 * Décompose les participants en groupes prêts à afficher.
 * Renvoie null si personne n'est renseigné, pour ne rien afficher du tout.
 */
export function participantGroups(participants, seed = "") {
  if (!participants) return null;
  const { adults, children } = participants;
  const groups = [];

  if (adults?.count) {
    groups.push({
      group: "adults",
      count: adults.count,
      emoji: participantEmoji("adults", seed),
      label: participantLabel("adults", adults.count),
      ages: null,
    });
  }

  if (children?.count) {
    groups.push({
      group: "children",
      count: children.count,
      emoji: participantEmoji("children", seed),
      label: participantLabel("children", children.count),
      ages: formatAges(children.ages),
    });
  }

  return groups.length > 0 ? groups : null;
}

/** Version texte, utile pour un titre de page ou un email : "2👩 / 3👧 (8, 11 et 13 ans)". */
export function formatParticipants(participants, seed = "") {
  const groups = participantGroups(participants, seed);
  if (!groups) return null;
  return groups
    .map((g) => `${g.count}${g.emoji}${g.ages ? ` (${g.ages})` : ""}`)
    .join(" / ");
}

/** "8, 11 et 13 ans" */
export function formatAges(ages) {
  if (!ages || ages.length === 0) return null;
  if (ages.length === 1) return `${ages[0]} ans`;
  return `${ages.slice(0, -1).join(", ")} et ${ages[ages.length - 1]} ans`;
}

// ---------------------------------------------------------------------------
// Matériel
//
// La déduction du pictogramme vit dans lib/gear.js, qui est le seul endroit du
// site à la faire. Le ré-export garde les anciens imports valides.
// ---------------------------------------------------------------------------
export { gearEmoji, GEAR_EMOJI_CHOICES, GEAR_FALLBACK, isKnownGearEmoji } from "./gear";

// ---------------------------------------------------------------------------
// Couleurs des étapes
// ---------------------------------------------------------------------------
// Douze teintes plutôt que six : une sortie peut compter jusqu'à trente étapes,
// et deux étapes voisines de même couleur sont indistinguables sur la carte.
// Les six premières ne bougent pas, les sorties déjà en ligne gardent leur allure.
export const STAGE_COLORS = [
  "#c1542d", "#d9a441", "#6f9464", "#4f7fa8", "#96628c", "#3f8f8a",
  "#b0476a", "#7f8f3a", "#5d69b0", "#a86b3c", "#3f7f5f", "#7c6f9c",
];

export function stageColor(index) {
  return STAGE_COLORS[index % STAGE_COLORS.length];
}

// ---------------------------------------------------------------------------
// Intitulé d'une étape
// ---------------------------------------------------------------------------

// « Jour 3 : vers Maaseik », « Étape 3 », « J3 », « 3e jour »… : le titre porte
// déjà son propre numéro, ajouter le nôtre donnerait « Étape 3 · Jour 3 ».
const NUMBER_FIRST = /^\s*(?:jours?|étapes?|etapes?|days?|stages?|j\.?)\s*n?[°º]?\s*\d+/i;
// Le pluriel est volontairement exclu ici : « 2 jours en Ardenne » est un titre,
// pas une numérotation. « Jours 4 et 5 » en est une, d'où le pluriel plus haut.
const NUMBER_LAST = /^\s*\d+\s*(?:er|re|ère|ere|ème|eme|e)?\s*(?:jour|étape|etape|day|stage)\b/i;

/** Le titre annonce-t-il déjà son rang dans l'itinéraire ? */
export function isSelfNumbered(title) {
  const text = String(title || "").trim();
  return NUMBER_FIRST.test(text) || NUMBER_LAST.test(text);
}

/**
 * Intitulé affiché pour une étape : « Étape 2 · Vers Maaseik ».
 *
 * Dès qu'une sortie compte plusieurs étapes, le numéro passe devant le titre.
 * Sans lui, une liste de noms de lieux ne dit pas dans quel ordre on roule, et
 * le rapprochement avec la couleur sur la carte se fait de mémoire.
 *
 * Sauf si celui qui propose la sortie a déjà numéroté ses étapes lui-même :
 * son intitulé est alors repris tel quel, sans double numérotation.
 *
 * Une sortie à trace unique n'a rien à numéroter : son intitulé ne bouge pas.
 */
export function stageLabel(title, index, total) {
  const numbered = `Étape ${index + 1}`;
  if (total <= 1) return title || numbered;
  if (!title) return numbered;
  return isSelfNumbered(title) ? title : `${numbered} · ${title}`;
}

// ---------------------------------------------------------------------------
// Mise en forme
// ---------------------------------------------------------------------------

/** "Belgique · Brabant flamand" */
export function formatPlace(country, region) {
  return [country, region].filter(Boolean).join(" · ");
}

/** Coupe proprement au dernier mot entier, sans casser une phrase en plein milieu. */
export function truncate(text, max = 155) {
  const clean = String(text || "").trim();
  if (clean.length <= max) return { text: clean, truncated: false };
  const cut = clean.slice(0, max);
  const lastSpace = cut.lastIndexOf(" ");
  return {
    text: `${cut.slice(0, lastSpace > max * 0.6 ? lastSpace : max).trimEnd()}…`,
    truncated: true,
  };
}
