...
### Retirer une sortie déjà publiée

Les pages du carnet sont statiques : elles existent sous forme de fichiers écrits au dernier build. Supprimer ou refuser une proposition dans `/admin` la retire du stockage, mais la page reste en ligne tant que le site n'a pas été reconstruit.

`Supprimer` et `Refuser` déclenchent donc le Deploy Hook quand la sortie était approuvée, exactement comme `Approuver`. Le message affiché dans `/admin` te dit ce qui s'est passé :

- reconstruction lancée : la sortie disparaît du carnet en une minute ou deux
- pas de `VERCEL_DEPLOY_HOOK_URL` configuré : elle reste visible jusqu'à ton prochain `git push`
- proposition jamais approuvée : rien à reconstruire, le carnet est déjà à jour

Une sortie ajoutée à la main dans `public/rides/` n'est pas concernée : elle vit dans le dépôt, il faut supprimer son dossier et pousser.
